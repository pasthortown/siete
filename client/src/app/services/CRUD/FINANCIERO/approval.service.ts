...
export class ApprovalService {
   ...
   url = environment.api_financiero + 'approval/';
   ...
   post(approval: Approval): Promise<any> {
      return this.http.post(this.url, JSON.stringify(approval), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(approval: Approval): Promise<any> {
      return this.http.put(this.url, JSON.stringify(approval), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
