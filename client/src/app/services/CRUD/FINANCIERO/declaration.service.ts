...
export class DeclarationService {
   ...
   url = environment.api_financiero + 'declaration/';
   ...
   post(declaration: Declaration): Promise<any> {
      return this.http.post(this.url, JSON.stringify(declaration), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(declaration: Declaration): Promise<any> {
      return this.http.put(this.url, JSON.stringify(declaration), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
