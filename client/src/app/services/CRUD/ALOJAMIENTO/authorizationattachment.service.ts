...
export class AuthorizationAttachmentService {

   url = environment.api_alojamiento + 'authorizationattachment/';
   ...
   post(authorizationattachment: AuthorizationAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(authorizationattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(authorizationattachment: AuthorizationAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(authorizationattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
