...
export class TariffService {
   ...
   url = environment.api_alojamiento + 'tariff/';
   ...
   post(tariff: Tariff): Promise<any> {
      return this.http.post(this.url, JSON.stringify(tariff), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(tariff: Tariff): Promise<any> {
      return this.http.put(this.url, JSON.stringify(tariff), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
