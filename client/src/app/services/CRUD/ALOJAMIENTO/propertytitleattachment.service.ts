...
export class PropertyTitleAttachmentService {

   url = environment.api_alojamiento + 'propertytitleattachment/';
   ...
   post(propertytitleattachment: PropertyTitleAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(propertytitleattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(propertytitleattachment: PropertyTitleAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(propertytitleattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
