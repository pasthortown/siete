...
export class ApprovalStateService {
   ...
   url = environment.api_alojamiento + 'approvalstate/';
   ...
   post(approvalstate: ApprovalState): Promise<any> {
      return this.http.post(this.url, JSON.stringify(approvalstate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(approvalstate: ApprovalState): Promise<any> {
      return this.http.put(this.url, JSON.stringify(approvalstate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
