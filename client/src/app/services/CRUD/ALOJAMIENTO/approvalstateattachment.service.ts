...
export class ApprovalStateAttachmentService {

   url = environment.api_alojamiento + 'approvalstateattachment/';
   ...
   post(approvalstateattachment: ApprovalStateAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(approvalstateattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(approvalstateattachment: ApprovalStateAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(approvalstateattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
