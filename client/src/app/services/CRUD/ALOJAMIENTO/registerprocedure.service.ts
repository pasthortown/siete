...
import { RegisterProcedure } from './../../../models/ALOJAMIENTO/RegisterProcedure';

@Injectable({
   providedIn: 'root'
})
export class RegisterProcedureService {

   url = environment.api_alojamiento + 'registerprocedure/';
   ...
   post(registerprocedure: RegisterProcedure): Promise<any> {
      return this.http.post(this.url, JSON.stringify(registerprocedure), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(registerprocedure: RegisterProcedure): Promise<any> {
      return this.http.put(this.url, JSON.stringify(registerprocedure), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
