...
export class InspectorReportService {
   ...
   url = environment.api_alojamiento + 'inspectorreport/';
   ...
   post(inspectorreport: InspectorReport): Promise<any> {
      return this.http.post(this.url, JSON.stringify(inspectorreport), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(inspectorreport: InspectorReport): Promise<any> {
      return this.http.put(this.url, JSON.stringify(inspectorreport), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
