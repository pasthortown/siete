...
export class InspectionAssigmentService {

   url = environment.api_alojamiento + 'inspectionassigment/';
   ...
   post(inspectionassigment: InspectionAssigment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(inspectionassigment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(inspectionassigment: InspectionAssigment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(inspectionassigment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
